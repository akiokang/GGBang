...
def video_parse_rgba(rgba_string):
    if isinstance(rgba_string, str) and rgba_string.startswith('rgba'):
        try:
            ...
            pass
    try:
        return ImageColor.getrgb(rgba_string), 1.0
    except ValueError:
        return (0, 0, 0), 0.5
...
def video_create_text_overlay(text, shared_style, specific_config, video_size):
    ...
    creation_args = {'font': font_file, 'fontsize': font_size, 'color': text_color, 'stroke_color': stroke_color,
    ...
    if zoom_percentages:
        ...
        if chosen_percentage != 0:
            ...
            if chosen_percentage > 0:
                ...
                processed_img = scaled_img.crop(
                    (int(crop_x), int(crop_y), int(crop_x + original_width), int(crop_y + original_height)))
...
def image_apply_text(img_path, text, config, pos_tuple, output_path, logger, stop_event):
    ...
    try:
        ...
        logger(f"❌ 打开图片失败: {img_path} - {e}"); return False
    img = image_preprocess(base_img, config, logger, img_path)
    draw = ImageDraw.Draw(img)
    W, H = img.size
    try:
        font = ImageFont.truetype(config["font_path"], config.get("font_size", 32))
    except Exception as e:
        logger(f"❌ 加载字体失败: {config['font_path']} - {e}"); img.save(output_path); return True
    font_color, bg_color = tuple(config.get("font_color", [0, 0, 0])), tuple(
        config.get("font_background_color", [255, 255, 255]))
    max_w_ratio, line_spacing = config.get("max_text_width_ratio", 0.8), random.choice(
        config.get("line_spacing_options", [10]))
    padding, align, radius = config.get("text_padding", 10), config.get("text_align_in_block", "center"), config.get(
        "corner_radius", 0)
    ...
    for i, line in enumerate(lines):
        ...
        if not no_background:
            bg_coords = (actual_text_x - padding, actual_text_y - padding, actual_text_x + line_w + padding,
                         actual_text_y + line_heights[i] + padding)
    ...
    try:
        img.save(output_path); logger(f"  ✅ 已写入：{output_path}"); return True
    except Exception as e:
        logger(f"❌ 保存图片失败: {output_path} - {e}"); return False
...
class App(ctk.CTk):
    ...
    def run_video_logic_hybrid(self):
        ...
        try:
            ...
            all_available_videos = [f for f in os.listdir(video_folder) if
            ...
            for i in range(0, len(videos_to_process), num_texts_per_group):
                ...
                for j, video_name in enumerate(video_chunk):
                    ...
                    output_video_path = os.path.join(group_folder,
                                                     f"processed_{os.path.splitext(video_name)[0]}.mp4")
                    ...
                    try:
                        ...
                        # --- 核心修改：生成PNG图片并构建FFmpeg命令 ---
                        ffmpeg_inputs = [f'-i "{corrected_video_path}"']
                        filter_chains = []
                        last_overlay_bottom_y = 0

                        # 假定一个视频尺寸用于生成高质量的PNG，例如1080p竖屏
                        dummy_video_size = (1080, 1920)

                        # 主文案
                        if text_parts:
                            self.log_video("  -> 步骤1: 使用生成主文案t...")
                            main_config = config['主文案']
                            png_path = os.path.join(group_folder, f"overlay_main_{random.randint(1000, 9999)}.png")

                            size = self._create_overlay_image_with_moviepy(
                                text_parts[0], shared_style, main_config, dummy_video_size, png_path
                            )
                            if not size: raise ValueError("主文案图片生成失败")

                            temp_overlay_files.append(png_path)
                            ffmpeg_inputs.append(f'-i "{png_path}"')
                        ...
                        if temp_video_file or temp_overlay_files:
                            self.log_video(f"  - 已清理所有临时文件。")
    ...
    def setup_greenscreen_composite_workflow(self):
        ...
        self.create_folder_selection_row(path_frame, "1. 选择绿幕文件夹:", "选择带绿幕的视频文件夹",
                                         "gs_composite_source_folder_entry")
        self.create_folder_selection_row(path_frame, "2. 选择视频文件夹:", "选择作为背景的视频文件夹",
                                         "gs_composite_bg_folder_entry")
        self.create_folder_selection_row(path_frame, "3. 选择输出文件夹:", "选择处理结果的存放位置",
                                         "gs_composite_output_folder_entry")
        ...
        ctk.CTkLabel(settings_frame, text="高级设置 (可选)", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0,
                                                                                                   columnspan=2,
                                                                                                   padx=10,
                                                                                                   pady=(5, 10),
                                                                                                   sticky="w")
        ...
        self.composite_start_button = ctk.CTkButton(button_frame, text="开始处理", height=40,
    ...
    def run_video_logic_ffmpeg(self):
        ...
        try:
            ...
            # (为了简洁，省略了之前添加的完整配置打印，您也可以保留它用于调试)
            ...
            shared_style = config['共享样式']  # 读取中文键
            group_count = 0
            for i in range(0, len(videos_to_process), num_texts_per_group):
                if self.video_stop_event.is_set(): break
                group_count += 1
                    ...
                    try:
                        ...
                        if text_parts:
                            ...
                            main_filter, main_box_h, temp_txt = self._create_ffmpeg_drawtext_filter(
                                text_parts[0], shared_style['字体文件'], shared_style['字体大小'],
                        ...
                        if len(text_parts) > 1:
                            sub_configs = config.get('次文案', [])  # 读取中文键
                            for k, sub_text in enumerate(text_parts[1:]):
                                if k >= len(sub_configs): break
                                sub_config = sub_configs[k]
                        ...
                        command_string = (
                            f'"{ffmpeg_path}" -y -i "{corrected_video_path}" '
                            f'-vf "{vf_string}" '
                            f'-c:v {output_codec} -preset {preset} -c:a aac -b:a 192k '
                            f'"{output_video_path}"'
                        )
    ...
    def setup_gs_replace_workflow(self):
        ...
        # --- 路径设置 ---
        ctk.CTkLabel(scrollable_frame, text="1. 设置路径", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10,
                                                                                                 pady=(10, 5))
        self.create_folder_selection_row(scrollable_frame, "背景视频文件夹:", "选择作为背景的视频文件夹",
                                         "gs_replace_bg_folder_entry")
        self.create_folder_selection_row(scrollable_frame, "绿幕视频文件夹:", "选择包含绿幕素材的视频文件夹",
                                         "gs_replace_gs_folder_entry")
        self.create_folder_selection_row(scrollable_frame, "输出文件夹:", "选择处理结果的存放位置",
                                         "gs_replace_output_folder_entry")

        # --- 参数设置 ---
        ctk.CTkLabel(scrollable_frame, text="2. 功能开关", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10,
                                                                                                 pady=(20, 5))
        ...
        self.gs_replace_start_button = ctk.CTkButton(button_frame, text="开始批量替换", height=40,
                                                     command=self.start_gs_replace_processing)
        self.gs_replace_start_button.grid(row=0, column=0, padx=(0, 5), sticky="ew")

        self.gs_replace_stop_button = ctk.CTkButton(button_frame, text="停止处理", height=40,
                                                    command=self.stop_gs_replace_processing, state="disabled",
                                                    fg_color="red", hover_color="darkred")
        self.gs_replace_stop_button.grid(row=0, column=1, padx=(5, 0), sticky="ew")

        # --- 日志区域 ---
        self.gs_replace_log_textbox = ctk.CTkTextbox(scrollable_frame, state="disabled", height=200,
                                                     text_color="#A9A9A9")
    ...
    def setup_freeze_workflow(self):
        ...
        self.create_folder_selection_row(tab, "源视频文件夹:", "选择包含视频的文件夹 (可含子文件夹)",
                                         "freeze_input_folder_entry")
        ...
        self.freeze_start_button = ctk.CTkButton(button_frame, text="开始批量定格处理", height=40,
                                                 command=self.start_freeze_processing)
        self.freeze_start_button.grid(row=0, column=0, padx=(0, 5), sticky="ew")

        self.freeze_stop_button = ctk.CTkButton(button_frame, text="停止处理", height=40,
                                                command=self.stop_freeze_processing, state="disabled", fg_color="red",
                                                hover_color="darkred")
    ...
    def setup_audio_extraction_workflow(self):
        ...
        self.create_folder_selection_row(tab, "音频输出文件夹:", "选择提取的MP3音频存放位置",
                                         "audio_output_folder_entry")
        ...
        self.start_audio_button = ctk.CTkButton(button_frame, text="开始批量提取音频", height=40,
                                                command=self.start_audio_extraction)
        self.start_audio_button.grid(row=0, column=0, padx=(0, 5), sticky="ew")

        self.stop_audio_button = ctk.CTkButton(button_frame, text="停止处理", height=40,
                                               command=self.stop_audio_extraction, state="disabled", fg_color="red",
                                               hover_color="darkred")
    ...
    def run_audio_extraction_logic(self):
        ...
        try:
            ...
            video_files = [f for f in os.listdir(input_dir) if
                           f.lower().endswith(('.mp4', '.avi', '.mkv', '.mov', '.flv', '.wmv'))]
            if not video_files:
    ...
    def setup_cut_workflow(self):
        ...
        ctk.CTkLabel(scrollable_frame, text="1. 设置路径", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10,
                                                                                                 pady=(5, 0))
        self.create_folder_selection_row(scrollable_frame, "源视频文件夹:", "选择包含视频的文件夹 (可含子文件夹)",
                                         "cut_input_folder_entry")
        self.create_folder_selection_row(scrollable_frame, "输出文件夹:", "选择处理结果的存放位置",
                                         "cut_output_folder_entry")

        # --- 2. 时间参数设置 ---
        ctk.CTkLabel(scrollable_frame, text="2. 设置卡秒时间点", font=ctk.CTkFont(weight="bold")).pack(anchor="w",
                                                                                                       padx=10,
                                                                                                       pady=(15, 0))
        ...
        # --- 3. FFmpeg 设置 (可选) ---
        ctk.CTkLabel(scrollable_frame, text="3. 高级设置 (可选)", font=ctk.CTkFont(weight="bold")).pack(anchor="w",
                                                                                                        padx=10,
                                                                                                        pady=(15, 0))
        self.create_widget_row(scrollable_frame, "FFmpeg.exe 路径:", "cut_ffmpeg_path",
                               "程序会自动查找，找不到时再手动指定", is_file=True)
        # 修改辅助函数，让其支持选择文件
        button = self.cut_ffmpeg_path_entry.master.winfo_children()[-1]  # 获取 "..." 按钮
        button.configure(command=lambda e=self.cut_ffmpeg_path_entry: self.select_file_for_entry(e, "选择 ffmpeg.exe",
                                                                                                 [("Executable files",
                                                                                                   "*.exe"),
                                                                                                  ("All files",
                                                                                                   "*.*")]))
        ...
        button_frame.pack(fill="x", pady=10)
        button_frame.grid_columnconfigure((0, 1), weight=1)
        self.cut_start_button = ctk.CTkButton(button_frame, text="开始批量卡秒处理", height=40,
                                              command=self.start_cut_processing)
        self.cut_start_button.grid(row=0, column=0, padx=(0, 5), sticky="ew")
        self.cut_stop_button = ctk.CTkButton(button_frame, text="停止处理", height=40, command=self.stop_cut_processing,
    ...
    def _cut_process_video(self, ffmpeg_path, input_path, output_path, start_time, end_time, jump_time):
        ...
        try:
            process = subprocess.run(command, check=True, stderr=subprocess.PIPE, stdout=subprocess.PIPE, text=True,
                                     encoding='utf-8')
    ...
    def setup_news_blur_workflow(self):
        ...
        ctk.CTkLabel(folders_frame, text="1. 选择文件夹", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10,
                                                                                                pady=(5, 10))
        self.create_folder_selection_row(folders_frame, "源视频文件夹:", "选择包含视频的文件夹",
                                         "blur_input_folder_entry")
        self.create_folder_selection_row(folders_frame, "输出文件夹:", "选择处理结果的存放位置",
                                         "blur_output_folder_entry")

        # --- 2. 功能开关与偏移量设置 ---
        settings_frame = ctk.CTkFrame(scrollable_frame)
        settings_frame.pack(fill="x", padx=5, pady=10)
        ctk.CTkLabel(settings_frame, text="2. 功能开关与偏移量", font=ctk.CTkFont(weight="bold")).pack(anchor="w",
                                                                                                       padx=10,
                                                                                                       pady=(5, 0))
        ...
        ctk.CTkLabel(text_frame, text="3. 自定义文案配置", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10,
                                                                                                 pady=(5, 0))
        ...
        self.create_option_menu_row(text_frame, "选择字体 (zt文件夹):", "blur_font", self.blur_font_list,
                                    self.blur_font_list[0] if self.blur_font_list else "")
        ...
        ctk.CTkLabel(run_frame, text="4. 开始处理", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10,
                                                                                          pady=(5, 0))

        button_frame = ctk.CTkFrame(run_frame, fg_color="transparent")
        button_frame.pack(fill="x", pady=10)
        button_frame.grid_columnconfigure((0, 1), weight=1)
        self.blur_start_button = ctk.CTkButton(button_frame, text="设置模板并开始批量处理", height=40,
                                               command=self.start_blur_processing)
        self.blur_start_button.grid(row=0, column=0, padx=(0, 5), sticky="ew")
        self.blur_stop_button = ctk.CTkButton(button_frame, text="停止处理", height=40,
                                              command=self.stop_blur_processing, state="disabled", fg_color="red",
                                              hover_color="darkred")
        ...
        ctk.CTkLabel(run_frame, text="重要提示：模板化处理假定所有视频的分辨率相同。", text_color="gray").pack(anchor="w",
                                                                                                             padx=10)
    ...
    def _blur_process_video(self, input_path, output_path, use_gpu, content_roi, pip_roi, text_config, offsets):
        ...
        try:
            ...
            sharp_foreground = original_clip.crop(x1=cx, y1=cy, width=cw, height=ch).set_position(
                ('center', cy + y_offset_main))
            ...
            if pip_roi:
                ...
                if first_frame is not None and (py + ph <= first_frame.shape[0]) and (px + pw <= first_frame.shape[1]):
                    ...
                    pip_clip = ImageClip(pip_image).set_duration(original_clip.duration).set_position(
                        (px, py - y_offset_pip))
            ...
            try:
                final_clip.write_videofile(output_path, codec=codec, audio_codec="aac", fps=(original_clip.fps or 24),
                                           threads=8, logger=None)
            except Exception as e_gpu:
                if 'nvenc' in codec:
                    self.log_blur(f"  -> 警告: GPU编码失败 ({e_gpu})。正在回退到CPU编码。")
                    final_clip.write_videofile(output_path, codec='libx264', audio_codec="aac",
                                               fps=(original_clip.fps or 24), threads=8, logger=None)
                else:
                    raise e_gpu
    ...
    def setup_news_greenscreen_workflow(self):
        ...
        ctk.CTkLabel(paths_frame, text="第一步: 设置路径", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10,
                                                                                                 pady=(5, 10))
        self.create_folder_selection_row(paths_frame, "内容文件夹 (无绿幕):", "选择包含无绿幕视频的文件夹",
                                         "news_content_folder_entry")
        self.create_folder_selection_row(paths_frame, "模板文件夹 (带绿幕):", "选择包含绿幕模板视频的文件夹",
                                         "news_template_folder_entry")
        self.create_folder_selection_row(paths_frame, "输出文件夹:", "选择处理结果的存放位置",
                                         "news_output_folder_entry")

        # --- 2. 字幕配置 (使用 .pack 布局) ---
        text_frame = ctk.CTkFrame(scrollable_frame)
        text_frame.pack(fill="x", padx=5, pady=(10, 5))
        ctk.CTkLabel(text_frame, text="第二步: 字幕/文案配置", font=ctk.CTkFont(weight="bold")).pack(anchor="w",
                                                                                                     padx=10,
                                                                                                     pady=(5, 0))
        ...
        self.create_option_menu_row(text_frame, "选择字体 (zt文件夹):", "news_font", self.news_font_list,
                                    self.news_font_list[0] if self.news_font_list else "")
        ...
        ctk.CTkLabel(gpu_frame, text="高级设置 (实验性功能)", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10,
                                                                                                    pady=5)

        switch_frame = ctk.CTkFrame(gpu_frame, fg_color="transparent")
        switch_frame.pack(fill="x", padx=10, pady=5)
        self.news_use_gpu_switch = ctk.CTkSwitch(switch_frame, text="启用GPU加速 (仅加速视频编码)",
                                                 command=self._news_toggle_gpu_options)
        ...
        ctk.CTkLabel(run_frame, text="第三步: 开始处理", font=ctk.CTkFont(weight="bold")).pack(anchor="w", padx=10,
                                                                                               pady=5)

        self.news_start_button = ctk.CTkButton(run_frame, text="设置模板并开始批量合成", height=40,
                                               command=self.start_news_greenscreen_processing)
    ...
    def _news_create_text_image(self, text, box_w, box_h, font_path, font_color, bg_color, corner_radius):
        ...
        try:
            font = ImageFont.truetype(font_path, box_h)
        except IOError:
            font = ImageFont.load_default()
            ...
            for word in words:
                ...
                if word_bbox[2] - word_bbox[0] > target_text_w:
                    ...
                    for char in word:
                        ...
                        if char_bbox[2] - char_bbox[0] <= target_text_w:
                            temp_word += char
                        else:
                    ...
                    if line_bbox[2] - line_bbox[0] <= target_text_w:
                        line += word + ' '
                    else:
        ...
        position = ((box_w - text_w) / 2,
                    (box_h - text_h) / 2 - final_bbox[1])  # Adjusted for better vertical centering
    ...
    def _news_process_video(self, content_path, template_path, output_path, content_roi, text_config, use_gpu=False,
        ...
        try:
            ...
            try:
                final_clip.write_videofile(output_path, codec=codec_to_use, audio_codec="aac",
                                           fps=(template_clip.fps or 24), threads=8, logger=None)
            except Exception as e_gpu:
    ...
    def start_news_greenscreen_processing(self):
        ...
        content_files = sorted(
            [f for f in os.listdir(content_dir) if f.lower().endswith(('.mp4', '.mov', '.avi', '.mkv'))])
    ...
    def create_folder_selection_row(self, parent, label_text, placeholder, attr_name, return_frame=False):
        ...
        open_button = ctk.CTkButton(frame, text="打开", width=60,
                                    command=lambda e=entry: self._open_folder_path(e.get()))
        open_button.grid(row=0, column=2, padx=5)
        select_button = ctk.CTkButton(frame, text="选择...", width=60,
                                      command=lambda e=entry, t=label_text: self.select_folder_for_entry(e, t))
    ...
    def create_widget_row(self, parent, label, name, val, is_file=False):
        ...
        if is_file:
            ctk.CTkButton(f, text="...", width=30, command=lambda e=e: self.select_file_for_entry(e, "选择字体文件",
                                                                                                  [("Font files",
                                                                                                    "*.otf *.ttf")])).grid(
    ...
    def log_video(self, message, clear=False):
        self.after(0, self._update_log, self.video_log_textbox, message, clear)

    def log_image(self, message, clear=False):
        self.after(0, self._update_log, self.image_log_textbox, message, clear)

    def log_ai_matting(self, message, clear=False):
        self.after(0, self._update_log, self.ai_matting_log_textbox, message, clear)

    def log_ab(self, message, clear=False):
        self.after(0, self._update_log, self.ab_log_textbox, message, clear)
    ...
    def setup_ai_matting_workflow(self):
        ...
        self.ai_bg_folder_row = self.create_folder_selection_row(tab, "背景图片文件夹:",
                                                                 "选择包含背景图片的文件夹 (pic)", "ai_bg_folder_entry",
                                                                 return_frame=True)
        ...
        self.ai_use_gpu_switch = ctk.CTkSwitch(options_frame, text="使用GPU加速 (需NVIDIA显卡)");
        self.ai_use_gpu_switch.pack(side="left", pady=10, padx=(0, 20))
        self.transparent_bg_switch = ctk.CTkSwitch(options_frame, text="输出透明背景视频 (不使用背景图片)",
                                                   command=self._toggle_bg_folder_state);
        self.transparent_bg_switch.pack(side="left", pady=10, padx=20)
        self.delete_bg_switch = ctk.CTkSwitch(options_frame, text="处理后删除用过的背景图 (请谨慎！)");
        self.delete_bg_switch.pack(side="left", pady=10, padx=20)
        button_frame = ctk.CTkFrame(tab, fg_color="transparent");
        button_frame.pack(fill="x", padx=10, pady=10, expand=False);
        button_frame.grid_columnconfigure((0, 1), weight=1)
        self.start_ai_matting_button = ctk.CTkButton(button_frame, text="开始AI抠像处理", height=40,
                                                     command=self.start_ai_matting);
        self.start_ai_matting_button.grid(row=0, column=0, padx=(0, 5), sticky="ew")
        self.stop_ai_matting_button = ctk.CTkButton(button_frame, text="停止处理", height=40,
                                                    command=self.stop_ai_matting, state="disabled", fg_color="red",
                                                    hover_color="darkred");
        self.stop_ai_matting_button.grid(row=0, column=1, padx=(5, 0), sticky="ew")
        self.ai_matting_log_textbox = ctk.CTkTextbox(tab, state="disabled", text_color="#A9A9A9");
        self.ai_matting_log_textbox.pack(expand=True, fill="both", padx=10, pady=10)
    ...
    def setup_frame_extractor_workflow(self):
        ...
        # --- UI控件 ---
        self.create_folder_selection_row(tab, "视频文件夹:", "选择包含视频的文件夹 (可含子文件夹)",
                                         "extractor_input_folder_entry")
        self.create_folder_selection_row(tab, "图片输出文件夹:", "选择截取图片的存放位置",
                                         "extractor_output_folder_entry")
        ...
        self.start_extractor_button = ctk.CTkButton(button_frame, text="开始截图", height=40,
                                                    command=self.start_frame_extraction)
        self.start_extractor_button.grid(row=0, column=0, padx=(0, 5), sticky="ew")

        self.stop_extractor_button = ctk.CTkButton(button_frame, text="停止处理", height=40,
                                                   command=self.stop_frame_extraction, state="disabled", fg_color="red",
                                                   hover_color="darkred")
    ...
    def setup_video_main_tab(self, tab):
        ...
        ctk.CTkLabel(group_frame, text="生成组数:", width=120, anchor="w").pack(side="left")
        self.video_num_groups_entry = ctk.CTkEntry(group_frame, placeholder_text="输入一个整数, 例如 5");
        self.video_num_groups_entry.pack(side="left", fill="x", expand=True)
        self.video_use_gpu_switch = ctk.CTkSwitch(tab, text="使用GPU加速编码 (需NVIDIA显卡)");
        self.video_use_gpu_switch.pack(anchor="w", padx=10, pady=5)
        button_frame = ctk.CTkFrame(tab, fg_color="transparent");
        button_frame.pack(fill="x", padx=10, pady=10);
        button_frame.grid_columnconfigure((0, 1), weight=1)
        self.start_video_button = ctk.CTkButton(button_frame, text="开始处理视频", height=40,
                                                command=self.start_video_processing_hybrid);
        self.start_video_button.grid(row=0, column=0, padx=(0, 5), sticky="ew")
        self.stop_video_button = ctk.CTkButton(button_frame, text="停止处理", height=40,
                                               command=self.stop_video_processing, state="disabled", fg_color="red",
                                               hover_color="darkred");
        self.stop_video_button.grid(row=0, column=1, padx=(5, 0), sticky="ew")
        self.video_log_textbox = ctk.CTkTextbox(tab, state="disabled", text_color="#A9A9A9");
        self.video_log_textbox.pack(expand=True, fill="both", padx=10, pady=10)
    ...
    def setup_image_main_tab(self, tab):
        ...
        group_frame.pack(fill="x", padx=10, pady=5)
        ctk.CTkLabel(group_frame, text="生成组数:", width=120, anchor="w").pack(side="left")
        self.image_num_groups_entry = ctk.CTkEntry(group_frame, placeholder_text="输入一个整数");
        self.image_num_groups_entry.pack(side="left", fill="x", expand=True)
        button_frame = ctk.CTkFrame(tab, fg_color="transparent");
        button_frame.pack(fill="x", padx=10, pady=10);
        button_frame.grid_columnconfigure((0, 1), weight=1)
    ...
    def setup_video_settings_tab(self, tab):
        ...
        for i in range(1, 3):
            ...
            defaults = [("20", "#FFD700", "black", "rgba(200,50,50,0.85)"),
                        ("30", "white", "black", "rgba(50,50,50,0.7)")]
        ...
        self.preview_button = ctk.CTkButton(tab, text="生成视频字幕预览", command=self.generate_video_preview);
        self.preview_button.grid(row=1, column=0, pady=10, padx=5)
        self.preview_display_frame = ctk.CTkFrame(tab);
        self.preview_display_frame.grid_rowconfigure(1, weight=1);
        self.preview_display_frame.grid_columnconfigure(0, weight=1)
        self.close_preview_button = ctk.CTkButton(self.preview_display_frame, text="关闭预览",
                                                  command=self.close_video_preview, width=120);
        self.close_preview_button.grid(row=0, column=0, pady=(5, 10))
        self.preview_label = ctk.CTkLabel(self.preview_display_frame, text="", text_color="gray");
    ...
    def setup_image_settings_tab(self, tab):
        ...
        scrollable_frame.pack(expand=True, fill="both", padx=5, pady=5)
        font_frame = ctk.CTkFrame(scrollable_frame, border_width=1);
        ...
        self.create_widget_row(font_frame, "字体文件路径:", "image_font_path", default_font_path, True);
        self.create_widget_row(font_frame, "字体大小:", "image_font_size", "75");
        self.create_color_picker_row(font_frame, "文字颜色:", "image_font_color", "#000000");
        self.create_color_picker_row(font_frame, "背景颜色:", "image_font_background_color", "#ffffff")
        ...
        self.create_textbox_row(list_frame, "缩放/裁剪百分比 (逗号分隔):", "image_zoom_crop_percentages",
                                "-3,-2,-1, 0, 10, 12,11,12,13,14")
        ...
        try:
            ...
            if background_path:
                try:
                    ...
                    if background_pil.size != video_size:
                        self.log_ai_matting(
                            f"    注意：背景图片尺寸 {background_pil.size} 与视频尺寸 {video_size} 不符，正在自动缩放...")
            ...
            try:
                ...
                if background_path:
                    ...
                    final_clip.write_videofile(output_path, codec='prores_ks', audio_codec='pcm_s16le', logger=None,
                ...
                try:
                    ...
                    if background_path:
                        ...
                        final_clip_no_audio.write_videofile(output_path, codec='prores_ks', logger=None,
                                                            ffmpeg_params=['-pix_fmt', 'yuva444p10le'])
    ...
    def run_ab_logic(self):
        try:
            ...
            self.log_ab(
                f"🔍 扫描结果: 发现背景图片 {len(source_images)} 张, 合规绿幕文件夹 {len(greenscreen_subfolders)} 个。")
            ...
            self.log_ab(
                f"▶️ 准备就绪: 将生成 {num_groups} 大组，每组处理所有 {len(greenscreen_subfolders)} 个绿幕文件夹。")
            ...
            for group_num in range(1, num_groups + 1):
                ...
                for gs_folder in greenscreen_subfolders:
                    ...
                    for gs_image_name in current_gs_images:
                        ...
                        self.log_ab(
                            f"    - 正在合成: {os.path.basename(bg_path)} + {gs_image_name} -> {output_filename}")
    ...
    def run_frame_extraction_logic(self):
        ...
        try:
            ...
            for i, video_path in enumerate(video_files):
                ...
                # 调用worker函数处理单个视频
                frames_count = self._frame_extraction_worker(video_path, output_folder, interval, resize_enabled,
                                                             use_gpu)
                if frames_count > 0:
    ...
    def _ab_worker(self, background_path, greenscreen_path, output_path):
        try:
            ...
            if background_img.size != greenscreen_img.size: greenscreen_img = greenscreen_img.resize(
                background_img.size, Image.Resampling.LANCZOS)
            ...
            self.log_ab(f"    ❌ 核心合成失败: {e}"); return False
    ...
    def get_image_config_from_gui(self):
        try:
            ...
            return {"font_path": self.image_font_path_entry.get(), "font_size": int(self.image_font_size_entry.get()),
                    ...
                    "line_spacing_options": spacing_list, "corner_radius": int(self.image_corner_radius_entry.get()),
            ...
            self.log_image(f"图片配置错误: {e}"); return None
    ...
    def generate_video_preview(self):
        ...
        try:
            config = self.get_video_config_from_gui()
            if not config: self.preview_label.configure(image=None,
                                                        text="配置无效，无法生成预览"); self.preview_display_frame.grid(
                row=2, column=0, sticky="nsew", padx=5, pady=5); return
            ...
            shared_style_config = config['shared_style']
            main_text_config = config['main_text']
            main_overlay_img = self._create_overlay_as_pillow_image(text_parts[0], shared_style_config,
                                                                    main_text_config, (1280, 720))
            pos_config = main_text_config['position']
            x_pos_val, y_pos = pos_config.get('x', 'center'), pos_config.get('y', 'center')
            paste_x = (preview_pil_image.width - main_overlay_img.width) // 2 if x_pos_val == 'center' else int(
                x_pos_val)
            paste_y = int(y_pos)
            ...
            for k, sub_text in enumerate(text_parts[1:]):
                if k >= len(sub_texts_configs): break
                sub_text_config = sub_texts_configs[k]
                sub_overlay_img = self._create_overlay_as_pillow_image(sub_text, shared_style_config, sub_text_config,
                                                                       (1280, 720))
                y_offset = sub_text_config.get("relative_y_offset", 10)
                new_y = last_clip_y + last_clip_height + y_offset
            ...
            ctk_image = ctk.CTkImage(light_image=preview_pil_image, dark_image=preview_pil_image,
    ...
    def _create_overlay_as_pillow_image(self, text, shared_style, specific_config, video_size):
        ...
        if single_line_width > max_pixel_width:
            ...
            for raw_line in manual_lines_raw:
                wrapped_lines, _, _ = image_wrap_text(dummy_draw, raw_line, font, max_pixel_width);
                final_lines.extend(wrapped_lines)
            text_for_drawing, align_for_drawing = '\n'.join(final_lines), 'center'
        else:
            text_for_drawing, align_for_drawing = probed_text, 'center'
        text_bbox = dummy_draw.multiline_textbbox((0, 0), text_for_drawing, font=font, align=align_for_drawing,
                                                  stroke_width=stroke_width)
        ...
        bg_size = (int(bg_width), int(bg_height));
        overlay_image = Image.new("RGBA", bg_size, (0, 0, 0, 0));
        ...
        if not no_background:
            ...
            bg_fill_color = bg_rgb + (int(bg_alpha * 255),)
            if corner_radius > 0:
                draw.rounded_rectangle(((0, 0), bg_size), int(corner_radius), fill=bg_fill_color)
            else:
                draw.rectangle(((0, 0), bg_size), fill=bg_fill_color)
        text_x, text_y = (bg_width - actual_text_width) / 2, (bg_height - actual_text_height) / 2
        draw.multiline_text((text_x, text_y - text_bbox[1]), text_for_drawing, fill=text_color, font=font,
                            align=align_for_drawing, stroke_width=stroke_width, stroke_fill=stroke_color)
    ...
    def save_settings(self):
        ...
        settings = {
            ...
            # 我们将之前隐藏的 audio 和 freeze 设置也一并注释掉
        }
        try:
    ...
    def load_settings(self):
        ...
        self.video_shared_font_file_entry.insert(0, vs.get('shared_font_file', ''))
        self.video_shared_size_entry.delete(0, 'end');
        self.video_shared_size_entry.insert(0, vs.get('shared_size', '60'))
        self.video_shared_max_width_ratio_entry.delete(0, 'end');
        self.video_shared_max_width_ratio_entry.insert(0, vs.get('shared_max_width_ratio', '0.9'))
        self.video_shared_padding_horizontal_entry.delete(0, 'end');
        self.video_shared_padding_horizontal_entry.insert(0, vs.get('shared_padding_horizontal', '30'))
        self.video_shared_padding_vertical_entry.delete(0, 'end');
        self.video_shared_padding_vertical_entry.insert(0, vs.get('shared_padding_vertical', '25'))
        self.video_shared_stroke_width_entry.delete(0, 'end');
        self.video_shared_stroke_width_entry.insert(0, vs.get('shared_stroke_width', '2'))
        self.video_shared_corner_radius_entry.delete(0, 'end');
        self.video_shared_corner_radius_entry.insert(0, vs.get('shared_corner_radius', '20'))
        # 新增下面几行
        ...
        self.video_main_pos_x_entry.insert(0, vs.get('main_pos_x', 'center'))
        self.video_main_pos_y_entry.delete(0, 'end');
        self.video_main_pos_y_entry.insert(0, vs.get('main_pos_y', '150'))
        ...
        self.video_sub1_offset_y_entry.insert(0, vs.get('sub1_offset_y', '20'))
        ...
        self.video_sub2_offset_y_entry.insert(0, vs.get('sub2_offset_y', '30'))
        ...
        self.image_text_input_box.insert("1.0", imgs.get('text_input', '图片文案'))
        ...
        self.image_font_path_entry.insert(0, imgs.get('font_path', ''))
        self.image_font_size_entry.delete(0, 'end');
        self.image_font_size_entry.insert(0, imgs.get('font_size', '75'))
        ...
        self.image_max_text_width_ratio_entry.insert(0, imgs.get('max_text_width_ratio', '0.85'))
        self.image_corner_radius_entry.delete(0, 'end');
        self.image_corner_radius_entry.insert(0, imgs.get('corner_radius', '15'))
        self.image_text_padding_entry.delete(0, 'end');
        self.image_text_padding_entry.insert(0, imgs.get('text_padding', '30'))
        ...
        self.image_text_positions_textbox.insert("1.0", imgs.get('text_positions', '[]'))
        self.image_line_spacing_options_textbox.delete("1.0", "end");
        self.image_line_spacing_options_textbox.insert("1.0", imgs.get('line_spacing_options', '45'))
        self.image_zoom_crop_percentages_textbox.delete("1.0", "end");
        self.image_zoom_crop_percentages_textbox.insert("1.0", imgs.get('zoom_crop_percentages', '0'))
        if imgs.get('allow_flip', True):
            self.image_flip_switch.select()
        else:
    ...
    def _update_color_widget(self, attr_name, color_value):
        setattr(self, attr_name + "_value", color_value); button = getattr(self,
                                                                           attr_name + "_button"); button.configure(
            text=str(color_value), fg_color="gray" if "rgba" in str(color_value) else color_value)
